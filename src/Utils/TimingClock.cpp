...
ClockSystem::ClockSystem() : m_time{ system_clock::now() } {}
ClockSystem::ClockSystem(const time_point& tp) : m_time{ tp } {}

time_point& ClockSystem::GetTimepoint() { 
    return this->m_time; 
}
long ClockSystem::GetDaysPassed() noexcept {
    return std::chrono::duration_cast<std::chrono::hours>(system_clock::now() - m_time).count() / 24;
}
int ClockSystem::GetHoursPassed() noexcept {
    return std::chrono::duration_cast<std::chrono::hours>(system_clock::now() - m_time).count();
}

std::string ClockSystem::GetString(const std::chrono::seconds& time) noexcept {
