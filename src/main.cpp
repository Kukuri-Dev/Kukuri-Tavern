...
int main(int argc, char* argv[]) {
    ...
    const char* Server_Name = "Kukuri Tavern";

    Logger::Init();
    Logger::Print(eLoggerType::INFO, fmt::format(fmt::emphasis::bold, "Starting {} {}",
    ...
    if (commandArguments.find("--nohttp") == std::string::npos) {
        if (!GetHTTP()->Listen())
            Logger::Print(eLoggerType::WARNING, "Failed to starting {}, please run an external service.", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "HTTP Server"));
    }
    if (!GetDatabase()->Connect()) {
        Logger::Print(eLoggerType::WARNING, "Failed to connect {}, please check server configuration.", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "SQLite Database"));
        return EXIT_FAILURE;
    }
    if (!GetItemManager()->Serialize()) {
        Logger::Print(eLoggerType::EXCEPTION, "{} >> failed to load items.dat, please make sure the file is on /cache", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "ItemManager"));
        return EXIT_FAILURE;
    }

    Logger::Print(eLoggerType::INFO, "Initializing {} and registering events...", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "Event Pool"));
    GetEventPool()->RegisterEvents();

    Logger::Print(eLoggerType::INFO, "Initializing {} and starting up {}...", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "Server Pool"), fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "Master Server"));
    if (enet_initialize() != 0) {
        Logger::Print(eLoggerType::EXCEPTION, fmt::format(fmt::emphasis::bold, "Failed to Initialize ENet, Terminating the Server..."));
        return EXIT_FAILURE;
    };
    if (!GetServerPool()->StartInstance()) {
        Logger::Print(eLoggerType::EXCEPTION, "Couldn't Startup {} Correctly, Terminating the Server...", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "Master Server"));
        return EXIT_FAILURE;
    };
    GetServerPool()->Start();
                ...
                case "shutdown"_fnv: {
                    for (auto [instanceId, server] : GetServerPool()->GetServers()) {
                        if (!GetServerPool()->StopInstance(instanceId))
                            Logger::Print(eLoggerType::WARNING, "Failed to shutdown InstanceId {}, this instance doesn't found on {}", fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "{}", instanceId), fmt::format(fmt::emphasis::bold | fg(fmt::color::cornsilk), "Server Pool"));
                    }
                } break;
                case "svinfo"_fnv: {
                    std::string data{};
                    ...
                    Logger::Print(eLoggerType::INFO, data);
                } break;
                case "hwinfo"_fnv: {
                    std::string
